...
public class OnStart : MonoBehaviour
{
    ...
    private static OnStart singleton;
    ...
    private IEnumerator Start()
    {
        ...
        if (BehaviourTestType == TestType.EudiActorBehaviour)
        {
            ...
            for (int i = 0; i < Cubes; i++)
            {
                ...
                newGo.AddComponent<CubeMovementComponent>().Data.Offset = i * 0.1f;
        ...
        {
            new GameObject("MonobehaviourManager").AddComponent<MonoBehaviourManager>();

            for (int i = 0; i < Cubes; i++)
            {
                var newGo = Instantiate(OriginalCube);
                newGo.AddComponent<CubeMovementMonoBehaviour>().Offset = i * 0.1f;
                CubesToBind.Add(newGo);
